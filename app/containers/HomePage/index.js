...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:3001/dlazdice')
      .then(response => {
  ...
  const handleAddNew = () => {
    ...
    setIsPending(true);
    axios.post('http://localhost:3001/dlazdice', newDlazdice).then(() => {
      setIsPending(false);
    });
  };
  ...
  const handleRemoveDlazdice = id => {
    ...
    axios
      .delete(`http://localhost:3001/dlazdice/${id}`)
  ...
  const handleSubmitSetting = e => {
    ...
    let textsToChange = [
      { extraText: extraText, mainText: mainText, layoutStyle: layoutStyle },
    ];
    axios
      .put(`http://localhost:3001/texts/0`, textsToChange)
      .then(() => console.log('texts changed on server'))
      .then(
        ...
        dlazdice.map(d => {
          axios
            .put(`http://localhost:3001/dlazdice/${d.id}`, d)
            .then(() => {
              //console.log(data)
      ...
      <Modal visible={visible} toggleVisibility={toggleVisibility}>
        <form className="settings" onSubmit={handleSubmitSetting}>
          <div className="settings__aside">
            ...
            <label htmlFor="display-style" className="d-block mb-1">
              Display
            </label>
            <select
              onChange={handleChangeLayout}
              name="display-style"
              id="display-style-select"
              value={layoutStyle}
          ...
          <div className="settings__dlazdice">
            <div className="d-flex justify-end">
              <a onClick={toggleVisibility} className="pull-right-10">
                <CloseIcon />
              </a>
            </div>
            ...
            <TableSetting
              classes="table responsive dlazdice-table"
              dlazdice={dlazdice}
